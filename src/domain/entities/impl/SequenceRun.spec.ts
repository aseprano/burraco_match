...
describe('SequenceRun', () => {
    ...
    it('can be snapshotted', () => {
        ...
        const run = SequenceRun.withCardsAndWildcardPosition(runCards, 3);
        run.setId(new RunID(8));
        ...
        expect(snapshot).toEqual({
            state: {
                entityId: 8,
                cards: [{suit: Suit.Clubs, value: 1}, {suit: Suit.Clubs, value: 2}, {suit: Suit.Clubs, value: 3}, {suit: Suit.Clubs, value: 2}],
                wildcardPosition: 3,
            },
            version: -1,
        });
    ...
    it('can be restored from snapshot', () => {
        ...
        run.restoreFromEventStream(
            ...
            },
            {
                state: {
                    entityId: 8,
                    cards: runCards.map((card) => { return { suit: card.getSuit(), value: card.getValue() }}),
                    wildcardPosition: 3
                },
                version: 10,
            }
        ...
        expect(run.getCards()).toEqual(runCards);
        expect(run.getWildcardPosition()).toEqual(3);
        expect(run.getId().asNumber()).toEqual(8);
    });
